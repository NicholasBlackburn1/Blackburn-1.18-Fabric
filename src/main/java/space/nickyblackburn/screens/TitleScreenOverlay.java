...
public class TitleScreenOverlay {
   ...
   public void setCopyRightTexT(String splash, MinecraftClient minecraft, int copyrightWidth, int copyrightX, int width,TextRenderer font,Screen screen){
       
      

        if (splash == null) {
            splash = minecraft.getSplashTextLoader().getName();
         }
   
         copyrightWidth = font.getWidth(Consts.copyright);
    ...
    public void drawCopyRightString(Screen screen, MatrixStack pose,TextRenderer  font, String copyrightinfo, int height, int width, int copyrightX, int copyrightWidth, int p_96740_, int p_96741_ , float f1){
      int l = MathHelper.ceil(f1 * 255.0F) << 24;

         screen.drawStringWithShadow(pose, font, copyrightinfo, copyrightX, height - 10, 16777215 | l);
    ...
    public void setDrawVersionName(MinecraftClient minecraft,Screen screen, MatrixStack pose,TextRenderer font, int height, int l ){

      String s = Consts.ReleaseName + SharedConstants.getGameVersion().getName();
      if (minecraft.isDemo()) {
         s = s + " Demo";
      } else {
      ...
      screen.drawCenteredText(pose, font, s, 2, height - 10, 16777215 | l);
      
    }

    // Sets the splash screen pos 
    public void setSplashPos(Screens screen ,String Splash, MatrixStack p_96739_, int width, int splashX, int splashrot, Font font, int l){
      /**
      if (Splash != null) {
         p_96739_.push();
         p_96739_.translate((double)(splashX), 70.0F, 1.0F);
         p_96739_.multiplyPositionMatrix(Matrix4f.translate((double)(splashX), y, z));
    ...
    */
   public void renderEdition(Screens screen,String splash, MatrixStack p_96739_, Font font, int width, int hight, int editionXfull,int editionYfull, int splashX, int j,int l, int editionXSmol, int editionYSmol, int editionImageWidth, int editionImageHight, int editionTextureWidth, int splashrot, boolean isfullscreen){
      int x = 0;
      int y =0;
   ...
   public void LoadCustomMainMenu(MinecraftClient minecraft, Screen titlescreen,int buttonwidth,int j){
      GuiUtils gui = new GuiUtils();   
      JsonPrimitive x,y ;
      ...
      gui.CreatebuttonwithoutImage(minecraft, titlescreen, new MultiplayerScreen(titlescreen), buttonwidth, j, x.getAsInt(), y.getAsInt(), 20, "menu.multiplayer");
